...
/**
 * 把外部传入的时间值转换成 Date。
 * Ledger 同时保存 validTime 和 transactionTime，统一 Date 类型可以避免 Mongo 中混入不可比较的字符串。
 */
...
/**
 * 追加一条记忆账本事件。
 * Ledger 是 append-only 账本，只记录发生过什么，不直接修改旧事件。
 */
...
/**
 * 记录本轮 Agent 在规划前读取了哪些记忆视图。
 * 这让 Trace 和 Ledger 都能解释“本次规划参考了哪些偏好和上下文”。
 */
async function recordMemoryRead({ userId, runId, views }) {
  return appendLedgerEvent({
    ...
    reason: 'Agent 运行前读取 Memory Views，用于 Trace 展示和记忆系统溯源。',
    actor: 'agent'
  });
}
 ...
 * 当前偏好仍由 AgentMemoryModel 保存，Ledger 负责记录为什么这样处理。
 */
...
/**
 * 记录成功工具链，供 toolSuccessView 展示历史执行事实。
 * 这里只保存工具名和结果工具列表，避免 Ledger 事件过大。
 */
...
/**
 * 从当前记忆和 Ledger 实时组装结构化 Views。
 * Views 不再单独落库，避免和 AgentMemoryModel 重复保存当前偏好与最近对话。
 */
async function buildMemoryViews(userId) {
...
/**
 * 刷新 Memory Views 的语义现在是“重新计算并记录一次刷新事件”。
 * 返回值是实时计算结果，不再写入独立的视图模型。
 */
async function refreshMemoryViews(userId) {
  const views = await buildMemoryViews(userId);
  await appendLedgerEvent({
    userId,
    eventType: 'view_refresh',
    memoryType: 'system',
    payload: {
      profileTopicCount: views.profileView.topics.length,
      timelineCount: views.timelineView.length,
      toolSuccessCount: views.toolSuccessView.length
    },
...
/**
 * 获取当前用户 Memory Views。
 * 这里只做实时计算，不追加 Ledger，避免前端刷新页面导致账本膨胀。
 */
async function getMemoryViews(userId) {
  return buildMemoryViews(userId);
}

/**
 * 查询当前用户 Ledger，按 transactionTime 倒序返回。
 * 前端用它展示原始账本和 Policy 决策过程。
 */
...
/**
 * 用户主动遗忘某类记忆。
 * 这里不删除历史 Ledger，而是追加 memory_forget 事件，保留可追溯性。
 */
